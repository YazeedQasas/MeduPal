...
export function InteractiveGlobe({
  ...
  arcColor = "rgba(100, 170, 145, 0.5)",
  markerColor = "rgba(120, 200, 175, 1)",
  glowColor = "rgba(80, 140, 120, 0.06)",
  strokeColor = "rgba(100, 170, 145, 0.12)",
  ...
  const draw = useCallback(() => {
    ...
    const glowOuter = radius * 1.18;
    const glowGrad = ctx.createRadialGradient(
      cx, cy, radius * 0.75,
      cx, cy, glowOuter
    );
    glowGrad.addColorStop(0, glowColor);
    glowGrad.addColorStop(1, "rgba(80, 140, 120, 0)");
    ...
    for (const marker of markers) {
      ...
      const pulseAlpha = markerColor.includes("1)") ? markerColor.replace("1)", `${0.2 + pulse * 0.15})`) : markerColor;
      ctx.strokeStyle = pulseAlpha;
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = markerColor.replace("1)", "0.6)").replace(" 1)", " 0.6)");
        ctx.fillText(marker.label, sx + 8, sy + 3);
      }
    }

    animRef.current = requestAnimationFrame(draw);
  }, [dotColor, arcColor, markerColor, glowColor, strokeColor, autoRotateSpeed, connections, markers]);
